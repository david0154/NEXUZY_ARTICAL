...
class LocalDatabase:
    ...
    def update_last_login(self, user_id: str) -> bool:
        ...
        try:
            self.cursor.execute(f"""
                UPDATE {USERS_TABLE} SET last_login = ? WHERE id = ?
    ...
    def add_article(self, article: Article) -> bool:
        ...
        try:
            ...
            self.connection.commit()
            logger.info(f"Article added: {article.article_name}")
            return True
        except Exception as e:
        ...
        try:
            self.cursor.execute(f"""
                UPDATE {ARTICLES_TABLE} SET 
                    article_name = ?, mould = ?, size = ?, gender = ?, updated_at = ?, sync_status = ?
                WHERE id = ?
            """, (
                article.article_name,
                article.mould,
            ...
            self.connection.commit()
            logger.info(f"Article updated: {article.article_name}")
            return True
        except Exception as e:
    ...
    def delete_article(self, article_id: str) -> bool:
        ...
        try:
            self.cursor.execute(f"""
                DELETE FROM {ARTICLES_TABLE} WHERE id = ?
            """, (article_id,))
            self.connection.commit()
    ...
    def get_pending_articles(self) -> List[Article]:
        ...
        try:
            self.cursor.execute(f"""
                SELECT * FROM {ARTICLES_TABLE} WHERE sync_status = ? ORDER BY created_at ASC
            """, (SYNC_PENDING,))
            articles = []
            for row in self.cursor.fetchall():
                articles.append(Article(
    ...
    def mark_article_synced(self, article_id: str) -> bool:
        ...
        try:
            self.cursor.execute(f"""
                UPDATE {ARTICLES_TABLE} SET sync_status = ? WHERE id = ?
            """, (SYNC_SYNCED, article_id))
            self.connection.commit()
            logger.info(f"Article marked as synced: {article_id}")
            return True
        except Exception as e:
    ...
    def get_user_articles(self, user_id: str) -> List[Article]:
        ...
        try:
            self.cursor.execute(f"""
                SELECT * FROM {ARTICLES_TABLE} WHERE created_by = ? ORDER BY created_at DESC
            """, (user_id,))
            articles = []
            for row in self.cursor.fetchall():
                articles.append(Article(
    ...
    def get_pending_articles_count(self) -> int:
        """Get count of articles pending sync"""
        try:
            self.cursor.execute(f"""
                SELECT COUNT(*) FROM {ARTICLES_TABLE} WHERE sync_status = ?
            """, (SYNC_PENDING,))
            return self.cursor.fetchone()[0]
        except Exception as e:
